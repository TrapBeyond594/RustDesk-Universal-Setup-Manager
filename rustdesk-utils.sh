...
pro_panel_info() {
  ...
  WAN_IP=$(curl -s -4 https://api64.ipify.org || echo "IP не определен")
  ...
  whiptail --title "Информация о панели Pro" --msgbox "URL: http://$WAN_IP:21114\nАдмин: $ADMIN_USER\nПароль: $ADMIN_PASS\n\nДля сброса пароля используйте 'rustdesk-utils set_password admin new_pass'" 15 60
}

change_address() {
  NEW_ADDR=$(whiptail --title "Смена адреса" --inputbox "Введите новый IP или Домен:" 10 60 3>&1 1>&2 2>&3)
  [ -z "$NEW_ADDR" ] && return

  whiptail --msgbox "Адрес $NEW_ADDR сохранен (информативно). В бесплатной версии адрес используется только в инсталляторах. В Pro - в настройках панели." 10 60
}
